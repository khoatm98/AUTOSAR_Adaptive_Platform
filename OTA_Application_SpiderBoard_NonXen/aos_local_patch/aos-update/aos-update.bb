...
CR52_IMAGE_FILE = "${BUNDLE_CR52_ID}-${BUNDLE_CR52_TYPE}-${CR52_IMAGE_VERSION}.gz"
...
python do_create_metadata() {
    ...
    if d.getVar("BUNDLE_CR52_TYPE") == "full":
        components_metadata.append(create_component_metadata(d.getVar("BUNDLE_CR52_ID"), d.getVar("CR52_IMAGE_FILE"),
            d.getVar("CR52_IMAGE_VERSION"), d.getVar("BUNDLE_CR52_DESC")))
    elif d.getVar("BUNDLE_CR52_TYPE"):
        bb.fatal("Wrong CR52 image type: %s" % d.getVar("BUNDLE_CR52_TYPE"))
...
do_create_cr52_image() {
    if [ -z ${BUNDLE_CR52_TYPE} ]; then
        exit 0
    fi

    if [ ! -f ${CR52_IMAGE_PATH} ]; then
        bbfatal "CR52 image ${CR52_IMAGE_PATH} not found"
    fi

    gzip < ${CR52_IMAGE_PATH} > ${BUNDLE_WORK_DIR}/${CR52_IMAGE_FILE}
}
...
python do_create_bundle() {
    ...
    bb.build.exec_func("do_create_cr52_image", d)
